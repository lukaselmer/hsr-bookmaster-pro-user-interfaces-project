package com.community.xanadu.components.coverflow;
...
public class Test01CoverFlow extends JPanel {
	...
	public void addItem(final Item i) {
		...
		i.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(final MouseEvent e) {
				if (index == Test01CoverFlow.this.selectedIndex) {
					if (i.action != null) {
		...
		ThreadUtils.invokeLater(new Runnable() {
			...
			public void run() {
				...
				getScrollBar().setMaximum(Test01CoverFlow.this.items.size());
			}
		});
	}
	...
	public void moveTo(final int index) {
		...
		if (this.anim.getState() == TimelineState.PLAYING_FORWARD) {
			return;
		}
	...
	private void placeOneitem(final int i) {
		...
		if (i < this.selectedIndex) {// left
			if (this.goingNext) {
				if (this.selectedIndex == i + 1) {
					xoffset = -((this.selectedIndex - i) + (this.animProgress)) * (oneStep - ((1f - this.animProgress) * (ItemWidthSize) / 2));
					if (xoffset > 0) {
						xoffset = 0;
					}
				} else {
		...
		this.items.get(i).setDelta(Math.abs(this.selectedIndex - i));
	...
	public void placeItems() {
		ThreadUtils.invokeLater(new Runnable() {
			...
			public void run() {
				...
				for (int i = Test01CoverFlow.this.selectedIndex; i >= 0; i--) {
					placeOneitem(i);
				}
				for (int i = Test01CoverFlow.this.selectedIndex + 1; i < Test01CoverFlow.this.items.size(); i++) {
					placeOneitem(i);
				}
			}
		});
	...
	public void moveToNext() {
		if (this.selectedIndex < this.items.size() - 1) {
			...
			ThreadUtils.invokeLater(new Runnable() {
				@Override
				public void run() {
					getScrollBar().setValue(Test01CoverFlow.this.selectedIndex);
				}
			});
		}
	}
	...
	private final TimelineCallback targetNext = new TimelineCallback() {
		...
		public void onTimelinePulse(final float durationFraction, final float timelinePosition) {
			ThreadUtils.invokeLater(new Runnable() {
				@Override
				public void run() {
					Test01CoverFlow.this.animProgress = timelinePosition;
					Test01CoverFlow.this.items.get(Test01CoverFlow.this.selectedIndex).setAngle(timelinePosition * angleLeft + angleRight);
					Test01CoverFlow.this.items.get(Test01CoverFlow.this.selectedIndex - 1).setAngle(timelinePosition * angleLeft);
					placeItems();
				}
			});
		}
	...
	private final TimelineCallback targetPrevious = new TimelineCallback() {
		...
		public void onTimelinePulse(final float durationFraction, final float timelinePosition) {
			ThreadUtils.invokeLater(new Runnable() {
				@Override
				public void run() {
					Test01CoverFlow.this.animProgress = 1 - timelinePosition;
					Test01CoverFlow.this.items.get(Test01CoverFlow.this.selectedIndex).setAngle((1 - timelinePosition) * angleLeft);
					Test01CoverFlow.this.items.get(Test01CoverFlow.this.selectedIndex + 1).setAngle(Math.PI - timelinePosition * angleLeft);
					placeItems();
				}
			});
		}
	...
	public void moveToPrevious() {
		if (0 < this.selectedIndex) {
			...
			this.anim = new Timeline();
			this.anim.setDuration(animDuration);
	...
	public static class Item extends JPanel {
		...
		private final float animProgress = 1f;

		public void setDelta(final int delta) {

			if (this.delta != delta && delta < 5) {
				if (this.timelineSelected != null && this.timelineSelected.getState() == TimelineState.PLAYING_FORWARD) {
					this.timelineSelected.cancel();
				}
				this.timelineSelected = new Timeline(this);
				this.timelineSelected.setDuration(animDuration);
				final int toRemove = (int) (50 * this.animProgress * ((float) delta / 5));
				this.timelineSelected.addPropertyToInterpolate("remove", this.remove, toRemove);
				this.timelineSelected.play();
			}
		...
		public Item(final BufferedImage buff, final AbstractAction action) {
			...
			this.interImage = GraphicsUtilities.createCompatibleTranslucentImage(this.front.getWidth(), this.front.getHeight()
					+ (this.PERSPECTIVE * 2));
			this.shadedImage = GraphicsUtilities.createCompatibleTranslucentImage(this.front.getWidth(), this.front.getHeight());
		}

		int remove;

		public void setRemove(final int remove) {
			this.remove = remove;
			repaint();
		}
		...
		protected void paintComponent(final Graphics g) {
			...
			final int dx0 = x0 + this.remove / 2;
			final int dy0 = y0 + this.remove / 2;

			final int w = this.interImage.getWidth() - this.remove;
			final int h = this.interImage.getHeight() - this.remove;

			g.drawImage(this.interImage, dx0, dy0, w, h, null);
		...
		private void updateImage() {
			...
			final BufferedImage image = this.front;
			...
			final GradientPaint gp = new GradientPaint(0, 0, new Color(0, 0, 0, 0), image.getWidth(), 0, new Color(0, 0, 0, alpha));
			g2.setPaint(gp);
