...
import com.community.xanadu.demo.components.TableAndActionsDemo;

public class PanelTableWithOverlayButtons extends JPanel {
	private static final long serialVersionUID = 1L;

	public static void main(final String[] args) {
		TableAndActionsDemo.main(args);
	}
	...
	private final int spaceForButtons;

	public PanelTableWithOverlayButtons(final JTable table) {
		this(table, 110);
	}

	public PanelTableWithOverlayButtons(final JTable table, final int spaceForButtons) {
		this.table = table;
	...
	private void showButton() {
		...
		final int rowH = this.table.getRowHeight();

		int xoffset = 0;

		for (final JButton b : getButton()) {
			final int y = row * rowH - (b.getHeight() - rowH) / 2;
			final int x = this.scroll.getWidth() + this.buttonXOffset + xoffset;
			final Point p = SwingUtilities.convertPoint(this.table, x, y, getPanelButtons());
	...
	private JPanel getPanelTable() {
		if (this.panelTable == null) {
			...
			this.panelTable.setLayout(new MigLayout("fill,inset 0 0 0 0"));
	...
	public void setButtons(final JButton[] _buttons) {
		if (this.buttons != null && this.buttons.length > 0) {
			for (final JButton b : this.buttons) {
				this.panelButtons.remove(b);
			}
		}

		if (_buttons != null) {
			for (final JButton b : _buttons) {
				this.panelButtons.add(b);
				b.setVisible(false);
	 ...
	 * if(buttonXOffset>0) there will be a gap between the buttons and the JTable
	 * 
	 * @param buttonXOffset
	 */
