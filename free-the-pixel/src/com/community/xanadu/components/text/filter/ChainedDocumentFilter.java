...
/**
 * A single DocumentFilter can be set on an AbstractDocument. There may be times when you wish to perform multiple
 * filtering of the data as it is added to a Document. By extending the ChainedDocumentFilter instead of the
 * DocumentFilter you will receive added flexibility in that the filter can be used stand alone or with other
 * ChainedDocumentFilters.
 * 
 * Whenever one filter step fails, the chaining of the filters is also terminated. In this case is it recommended you
 * invoke the provideErrorFeedback() method to provide user feedback.
 */
