 ...
 * <p/>
 * The principle is to record the time at which we start processing an event, and have another thread check frequently
 * to see if we're still processing. If the other thread notices that we've been processing a single event for too long,
 * it prints a stack trace showing what the event dispatch thread is doing, and continues to time it until it finally
 * finishes.
 * <p/>
 ...
 * <a href= "http://elliotth.blogspot.com/2005/05/automatically-detecting-awt-event.html" > Automatically detecting AWT
 * event dispatch thread hangs</a>
 * </p>
 * 
...
public final class EventDispatchThreadHangMonitor extends EventQueue {
	...
	private static class DispatchInfo {
		...
		private static boolean stackTraceElementIs(final StackTraceElement e, final String className, final String methodName, final boolean isNative) {
		...
		private void examineHang() {
			...
			Log.warn("(hang #" + this.hangNumber + ") event dispatch thread stuck processing event for " + timeSoFar() + " ms:" + stackTrace);
			checkForDeadlock();
		}
	...
	protected void dispatchEvent(final AWTEvent event) {
		try {
			...
			postDispatchEvent();
			if (!this.haveShownSomeComponent && event instanceof WindowEvent && event.getID() == WindowEvent.WINDOW_OPENED) {
	...
	private static void checkForDeadlock() {
		...
		for (ThreadInfo info : threadInfos) {
			Log.warn("Thread #" + info.getThreadId() + " " + info.getThreadName() + " (" + info.getThreadState() + ") waiting on "
					+ info.getLockName() + " held by " + info.getLockOwnerName() + stackTraceToString(info.getStackTrace()));
		}
	}
