...
public class EventDispatchThreadViolationRepaintManager extends RepaintManager {
	...
	private void checkThreadViolations(final JComponent c) {
		if (!SwingUtilities.isEventDispatchThread() && c.isShowing()) {
			Exception exception = new UiThreadingViolationException("Component methods call must be done on the Event Dispatch Thread");
			...
			for (StackTraceElement st : stackTrace) {
				...
				if (st.getClassName().startsWith("javax.swing.text.JTextComponent") && st.getMethodName().equals("setText")) {
