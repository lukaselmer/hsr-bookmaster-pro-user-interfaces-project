		...
		anim.addCallback(new TimelineCallback() {
			...
			public void onTimelineStateChanged(final TimelineState oldState, final TimelineState newState, final float durationFraction,
				...
				if (newState == TimelineState.DONE) {
					...
					((Demo2AnimatedCellRenderer) Demo2AnimatedJList.this.jlist.getCellRenderer()).setAnimProgress(i, 1);
			...
			public void onTimelinePulse(final float durationFraction, final float timelinePosition) {
				((Demo2AnimatedCellRenderer) Demo2AnimatedJList.this.jlist.getCellRenderer()).setAnimProgress(i, 1 - timelinePosition);
				Demo2AnimatedJList.this.jlist.repaint();
			}
		});
	...
	final public void addElement(final String text, final String code) {
		...
		anim.addCallback(new TimelineCallback() {
			...
			public void onTimelineStateChanged(final TimelineState oldState, final TimelineState newState, final float durationFraction,
					final float timelinePosition) {
				if (newState == TimelineState.DONE) {
					((Demo2AnimatedCellRenderer) Demo2AnimatedJList.this.jlist.getCellRenderer()).setAnimProgress(i, timelinePosition);
					Demo2AnimatedJList.this.jlist.repaint();
				}
			}
		...
		SwingUtilities.invokeLater(new Runnable() {
			@Override
			public void run() {
				Demo2AnimatedJList.this.scroll.getVerticalScrollBar().setValue(Demo2AnimatedJList.this.scroll.getVerticalScrollBar().getMaximum());
			}
		});
	}
}
