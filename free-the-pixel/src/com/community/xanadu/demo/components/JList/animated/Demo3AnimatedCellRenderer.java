...
public class Demo3AnimatedCellRenderer extends DynamicListCellRenderer<MessageItem> {
	...
	private void prepareForAnim(final Float progress, final MessageItem item) {
		final Color foreground = SubstanceColorUtilities.getInterpolatedColor(SELECTED_FOREGROUND, NOT_SELECTED_FOREGROUND, progress);
		this.labelTitle.setForeground(foreground);
		this.labelInfo.setForeground(foreground);
		this.mainPanel.setBackground(SubstanceColorUtilities.getInterpolatedColor(SELECTED_BACKGROUND, NOT_SELECTED_BACKGROUND, progress));
		this.labelInfo.setText(item.getText());
		this.alphaPanel.setAlpha(progress);
	}

	@Override
	public Component getListCellRendererComponent(final DynamicSizeJList<MessageItem> list, final MessageItem value, final int index,
			final int selectingIndex, final int deselectingIndex, final float progress, final boolean cellHasFocus) {
		...
		if (index == selectingIndex && selectingIndex != -1) {
			...
			// deselecting
			prepareForAnim(progress, value);
			this.mainPanel.setPreferredSize(new Dimension(50, (int) (CELL_SELECTED_HEIGHT - (CELL_SELECTED_HEIGHT - CELL_UNSELECTED_HEIGHT)
					* (1 - progress))));
