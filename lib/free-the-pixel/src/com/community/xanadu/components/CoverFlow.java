...
public class CoverFlow extends JPanel {
		...
		public void onTimelinePulse(final float durationFraction, final float timelinePosition) {
			ThreadUtils.invokeLater(new Runnable() {
				...
				public void run() {
					...
					CoverFlow.this.items.get(CoverFlow.this.selectedIndex).setAngle(timelinePosition * angleLeft + angleRight);
					CoverFlow.this.items.get(CoverFlow.this.selectedIndex - 1).setAngle(timelinePosition * angleLeft);
					placeItems();
				}
			});
		}
		...
		public void onTimelinePulse(final float durationFraction, final float timelinePosition) {
			ThreadUtils.invokeLater(new Runnable() {
				...
				public void run() {
					...
					CoverFlow.this.items.get(CoverFlow.this.selectedIndex).setAngle((1 - timelinePosition) * angleLeft);
					CoverFlow.this.items.get(CoverFlow.this.selectedIndex + 1).setAngle(Math.PI - timelinePosition * angleLeft);
					placeItems();
				}
			});
		}
	...
	public static class Item extends JPanel {
		...
		protected void paintComponent(final Graphics g) {
			...
			g.setClip(imageX + x0, imageY + y0, this.interImage.getWidth() - (2 * x0) - 1, (this.interImage.getHeight() - (2 * y0)));
