...
public class TaskButton extends JButton {
	...
	private float shadowOffsetX = 1;
	private float shadowOffsetY = 1;
	...
	private void paintImage(final Graphics2D g2, final float y) {
		...
		if (this.ghostValue > 0.0f) {
			...
			if (this.clicked) {
				g2.setComposite(AlphaComposite.SrcOver.derive((float) (1.0f - this.ghostValue * 0.5)));
			} else {
				g2.setComposite(AlphaComposite.SrcOver.derive(0.5f * (1.0f - this.ghostValue)));
			}
	...
	private float paintText(final Graphics2D g2) {
		...
		TextLayout layoutSmallFont = new TextLayout(this.description == null ? " " : this.description, this.categorySmallFont, context);
		Rectangle r = getRectangle();
		{
			...
			g2.setColor(SubstanceColorUtilities.getInterpolatedColor(this.categoryHighlightColor, this.categoryColor, this.newFraction));
	...
	public void setCategoryHighlightColor(Color categoryHighlightColor) {
		this.categoryHighlightColor = categoryHighlightColor;
	}
}