...
public class CheckerManager {
	...
	/**
	 * return a component that give visual feedback if the component does not pass the checkers tests
	 * 
	 * @param <T>
	 * 
	 * @param compToTrack
	 ...
	 */
	public static <T extends JComponent> JComponent getComponentWithCheckers(final T compToTrack, final List<Checker<T>> checkers,
			final eViewError type, final Shape shape) {
		if (compToTrack == null) {
	...
	public static <T extends JComponent> JComponent getComponentWithCheckers(final T compToTrack, final Checker<T> checker, final eViewError type) {
	...
	private static <T extends JComponent> JComponent getBlinking(final JComponent overlayed, final T originalCompTotrack,
			final List<Checker<T>> checkers, final Shape shape) {
	...
	private static <T extends JComponent> JComponent getBlinking(final T compToTrack, final List<Checker<T>> checkers, final Shape shape) {
	...
	private static <T extends JComponent> JComponent getErrorOverlayedIcon(final List<Checker<T>> checkers, final JComponent comp,
			final T originalTrackedComp) {
	...
	public static class BlinkingLayer<T extends JComponent> extends AbstractLayerUI<JComponent> {
		...
		public BlinkingLayer(final JComponent extraComp, final T compToTrack, final List<Checker<T>> checkers, final Shape shape,
				final JXLayer<JComponent> jxlayer) {
			super();
	...
	public static class IconLayer<T extends JComponent> extends AbstractLayerUI<JComponent> {
		...
		public IconLayer(final JComponent comp, final T originalTrackedComp, final List<Checker<T>> checkers, final JComponent info) {
			super();
			this.checkers = checkers;
