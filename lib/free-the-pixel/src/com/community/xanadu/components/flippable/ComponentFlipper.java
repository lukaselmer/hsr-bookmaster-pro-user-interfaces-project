...
/**
 * A class to animate the visibility of two components by visually flipping them so it looks like one component is on
 * the back (flip side) of the other.
 * 
 * For the animation to look the best the components should be the same size and centered vertically.
 * 
 * The animcation takes place on a layered pane above the actual components. Call flip(...) with the the two components
 * to flip. The flip process will start the animation on the layer pane, call setVisible(false) on the first component,
...
public class ComponentFlipper {
	...
	public void flip(final JComponent frontComponent, final JComponent backComponent, final JLayeredPane layeredPane) {
		...
		this.renderComp.interImage = GraphicsUtilities.createCompatibleTranslucentImage(this.frontImage.getWidth(), this.frontImage.getHeight()
				+ (this.EXTRA_SPACE * 2));
		this.renderComp.shadedImage = GraphicsUtilities.createCompatibleTranslucentImage(this.frontImage.getWidth(), this.frontImage.getHeight());
	...
	private void doSizing() {
		...
		Point p = (this.frontComponent.isShowing()) ? this.frontComponent.getLocationOnScreen() : this.backComponent.getLocationOnScreen();
	...
	private BufferedImage grabImage(final JComponent comp, final Insets insets) {
		BufferedImage image = GraphicsUtilities.createCompatibleTranslucentImage(comp.getWidth(), comp.getHeight() + insets.top + insets.bottom);
	...
	/**
	 * The class where the intermediate images are rendered to. It is added to the layer pane, the animation is
	 * triggered and this component repaints the animation frames. The component is then removed after animation stops
	...
	private class RenderComponent extends JComponent {
		...
		public void paintComponent(final Graphics g) {
			...
			g2.setClip(imageX + x0, imageY + y0, this.interImage.getWidth() - (2 * x0) - 1, (this.interImage.getHeight() - (2 * y0)));
			if (this.rads < Math.PI / 2) {
				g2.drawImage(this.interImage, imageX + x0, imageY + y0, imageX + x0 + this.interImage.getWidth(), imageY + y0
						+ this.interImage.getHeight(), 0, 0, this.interImage.getWidth(), this.interImage.getHeight(), null);
			} else {
				g2.drawImage(this.interImage, imageX + this.interImage.getWidth() - x0, imageY + y0, imageX - x0, imageY + y0
						+ this.interImage.getHeight(), 0, 0, this.interImage.getWidth(), this.interImage.getHeight(), null);
			}
			g2.dispose();
		}
		...
		private void updateImage() {
			...
			GradientPaint gp = new GradientPaint(0, 0, new Color(0, 0, 0, 0), image.getWidth(), 0, new Color(0, 0, 0, alpha));
			g2.setPaint(gp);
			g2.fillRect(0, ComponentFlipper.this.EXTRA_SPACE / 2, image.getWidth(), image.getHeight() - ComponentFlipper.this.EXTRA_SPACE);
			g2.dispose();
