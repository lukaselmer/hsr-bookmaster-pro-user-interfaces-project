	...
	protected void paintImage(final Graphics g, final float animFraction) {
		if (this.buff == null) {
			if (this.renderingComp.getWidth() * this.renderingComp.getHeight() > (300 * 300)) {
				...
				this.originalImage = GraphicsUtilities.createThumbnail(this.originalImage, this.renderingComp.getWidth() / 2, this.renderingComp
						.getHeight() / 2);
			}
			this.buff = GraphicsUtilities.createCompatibleTranslucentImage(this.originalImage.getWidth(), this.originalImage.getHeight());
		}
