...
public class ValidationOverlay extends DefaultOverlayable {
	...
	public ValidationOverlay(final JXLayer<JComponent> comp, final ValidationLayer<JComponent> layer, final ValidationPanelIcon panelicon,
			final int location, final int xoffset, final int yoffset) {
	...
	public void setIcon(final BufferedImage newicon) {
		if (getOverlayComponents()[0] instanceof ValidationPanelIcon) {
			...
			((ValidationPanelIcon) getOverlayComponents()[0]).setPreferredSize(new Dimension(newicon.getWidth(), newicon.getHeight()));
			repaint();
		}
	}
