...
public class Demo2AnimatedCellRenderer extends JPanel implements ListCellRenderer {
			...
			if (index == list.getModel().getSize() - 1) {
				...
				if (list.getModel().getSize() - 2 == index) {
					if (this.mapAnimWarning.get(index + 1) != null) {
						...
						if (animProgress != 0) {
							this.color = (SubstanceColorUtilities.getInterpolatedColor(this.darkColor, this.lightColor, animProgress));
	...
	protected void paintComponent(final Graphics g) {
		...
		final GradientPaint gp = new GradientPaint(getWidth() / 2, 0, new Color(0, 0, 0, 50), getWidth() / 2, getHeight(),
